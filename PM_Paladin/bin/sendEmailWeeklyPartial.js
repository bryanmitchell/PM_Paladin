...
var getEngSsoQuery = `
	...
	INNER JOIN [WorkstationAssignedTo] wat ON ws.WorkstationID = tat.WorkstationID
...
new sql.Request(cp).query(getEngSsoQuery, function(err, recordset) {
	if(err) {console.log(err);} 
		...
		for(var i; i<recordset.length; ++i){
			new sql.Request(cp).query(engEmailQuery(recordset[i].sso), function(err, recordset) {
				if(err) { console.log(err); }
